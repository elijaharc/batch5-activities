/* Make Chess Functional!
 1 Moving pieces (according to the rules)
 2 Rules (White first, capturing enemy piece, taking turns)
 3 Timer
 4 En passant (https://en.wikipedia.org/wiki/En_passant)
 5 Castling (https://en.wikipedia.org/wiki/Castling) */
...
function createPiece(cell) {
  ...
  if (i == 0 || i == 1) {
    img.src = `images/white/${piece}.png`;
...
document.body.addEventListener("click", function (e) {
  if (
    (e.target.className === "piece" || e.target.classList.contains("cell")) &&
    gameOn
  ) {
    nextMove(e);
...
function inCheck() {
  ...
  opponent_pieces.forEach((piece) => {
    ...
    if (
      piece_type === "pawn" &&
      pawnMoves(piece).indexOf(king_coords.toString()) > -1
    ) {
      check = true;
    } else if (
      piece_type === "knight" &&
      knightMoves(piece).indexOf(king_coords.toString()) > -1
    ) {
      check = true;
    } else if (
      piece_type === "rook" &&
      rookMoves(piece).indexOf(king_coords.toString()) > -1
    ) {
      check = true;
    } else if (
      piece_type === "bishop" &&
      bishopMoves(piece).indexOf(king_coords.toString()) > -1
    ) {
      check = true;
    } else if (
      piece_type === "queen" &&
      queenMoves(piece).indexOf(king_coords.toString()) > -1
    ) {
      check = true;
    } else if (
      piece_type === "king" &&
      kingMoves(piece).indexOf(king_coords.toString()) > -1
    ) {
      check = true;
    }
  });
